...
| **tc-repo seed** (CircleCI) | 노드 `/home/tc-repo/` 아래의 git seed(blob:none partial + sparse clone). CircleCI task pod가 overlay lowerdir로 마운트해 재사용한다. 있는 것은 `cubrid-testcases-private-ex` 하나이고, **갱신 cron도 seed를 만드는 role도 코드에 없다** — 사람이 손으로 갱신한다(2026-08-19 실물 확인). CircleCI test_shell이 끝날 때까지 두고 그 뒤에 지운다. |
| **repo seed** (gha-ci) | 둘이다. ① 볼륨 `gha-ci` 의 `repos/` — `repos/<name>.git` bare mirror + `repos/<name>/` worktree seed, `gha-repo-seed` CronJob 이 갱신한다(ansible 관리 밖, 워크플로가 옮긴 뒤 티켓 25 가 지운다). ② 노드 사본 `/home/ci/seed/{build,test}` — 같은 모양을 워커마다 제 디스크에 두고 `gha-node-seed` DaemonSet 이 매시 갱신한다(`roles/arc/tasks/repo_seed.yml`, CUBRIDQA-1501 티켓 72). 워크플로는 cubrid PR B 부터 ② 를 읽는다. gha-ci 는 `/home/tc-repo` 를 읽지 않는다. |
