...
| **repo seed** (gha-ci) | GlusterFS 볼륨 `gha-ci`의 `repos/` 아래. `repos/<name>.git`은 bare mirror고 `repos/<name>/`은 그 미러에서 하드링크로 뜬 worktree seed다. `gha-repo-seed` CronJob이 갱신한다(`roles/arc/tasks/repo_seed.yml`). gha-ci는 이것만 읽고 `/home/tc-repo`를 읽지 않는다. |
