...
| **repo seed** (gha-ci) | 노드 사본 `/home/ci/seed/{build,test}` 하나다 — `<name>.git` bare mirror + `<name>/` worktree seed 를 워커마다 제 디스크에 두고 `gha-node-seed` DaemonSet 이 매시 갱신한다(`roles/arc/tasks/repo_seed.yml`, CUBRIDQA-1501 티켓 72). 볼륨 `gha-ci` 의 `repos/` 와 그것을 갱신하던 `gha-repo-seed` CronJob 은 티켓 25 가 지웠다(2026-09-22). gha-ci 는 `/home/tc-repo` 를 읽지 않는다. |
