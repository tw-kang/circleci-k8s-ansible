...
| **owner_email** | Per-host inventory variable that propagates as a Prometheus label → AlertManager `labels.owner_email` → Teams card mention via `msteams.entities`. M365 UPN inside the flow's tenant produces a real Teams notification; cross-tenant guest (gmail.com etc.) renders as plain `<at>local-part</at>` text only — no notification fires. Same constraint applied to the previous adapter path. |
| **test_purpose** | Per-host inventory variable describing the QA workload (e.g., `perf`, `regression`). Surfaces as a Prometheus label so dashboards can filter. |
