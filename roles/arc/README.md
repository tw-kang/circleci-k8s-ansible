...
| **`metadata.labels`** — `gha-ci.cubrid.org/lane` 추가 (2026-09-21) | 골든에 없는 라벨 한 줄. **주석이 아니라 값이 갈리는 항목이다** — 대조하면 렌더 줄 하나가 는 것으로 보인다 | 훅이 pod template 의 `metadata.labels` 를 job pod 에 병합한다(runner 이미지 `/home/runner/k8s/index.js` 의 `mergeObjectMeta`). job pod 을 lane 별로 가르는 유일한 칸이다 (티켓 17). kube-state-metrics 쪽 짝은 `monitoring.yml` 의 `metricLabelsAllowlist` 다 — 한쪽만 적용하면 지표가 안 갈린다. ⚠ job 종류·suite 는 이 자리에 못 담는다: `readExtensionFromFile` 이 `yaml.load` 만 하고 치환을 하지 않는다 |
